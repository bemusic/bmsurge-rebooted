...
const config = {
  ...
  MP3_URL_PATTERN: env('MP3_URL_PATTERN'),
}
...
fastify.get('/getSong', async (request, response) => {
  ...
  const requests =
    (await admin.database().ref('requests').once('value')).val() || {}
  const recentlyPlayed = new Set(
    Object.values(
      (await admin.database().ref('station/history').once('value')).val() || {}
    ).map((h) => h.songId)
  )
  ...
  if (requested) {
    await admin.database().ref('requests').child(fulfillableRequest).set(null)
  }
  const info = {
    song,
    requested,
  ...
  await admin.database().ref('station/next').update(getPublicSongPayload(info))
  return {
    url: `${config.MP3_URL_PATTERN.replace('%s', song.fileId)}`,
...
fastify.put('/putSong', async (request, response) => {
  if (!isAuthenticated(request)) {
    response.status(401).send('Unauthorized')
    return
  }
  ...
  await admin.database().ref('station').update(songPayload)

  const historyRef = admin.database().ref('station/history')

  // Add new entry to history
  await admin.database().ref('station/history').push(songPayload)
...
fastify.put('/updateSongDatabase', async (request, response) => {
  if (!isAuthenticated(request)) {
    response.status(401).send('Unauthorized')
    return
  }
  ...
  await admin.database().ref('songs').set(songs)
  await admin
    .storage()
    .bucket()
    .file('songlist.json')
  ...
  await admin
    ...
    .bucket()
    .file('songlist.json.gz')
    .save(zlib.gzipSync(Buffer.from(JSON.stringify(songlist)), { level: 9 }), {
      resumable: false,
    })
...
async function requestSong(userId, username, songId, query) {
  ...
  const s = (
    await admin.database().ref('songs').child(songId).once('value')
  ).val()
  if (s) {
    const requestRef = admin.database().ref('requests').child(s.songId)
    ...
    await requestRef.child('requesters').child(userIdHash).set(requestTime)
    ...
    await admin.database().ref('logs/requests').push({
      userId: userId,
      username: username,
      songId: s.songId,
      songText: songText,
...
function hashUserId(userId) {
  return require('crypto').createHash('md5').update(userId).digest('hex')
}

fastify.listen(57263, '0.0.0.0')
